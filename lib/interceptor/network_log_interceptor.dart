...
class NetworkLogInterceptor extends Interceptor {
  ...
  void _printResponse(Response response) {
    try {
      if (response.data != null) {
        if (response.data is NetworkResultSuccess) {
          _printNetworkResult(response.data as NetworkResultSuccess);
  ...
  void _printNetworkResult(
    NetworkResultSuccess result, {
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          ...
          _printMap(value, initialTab: tabs);
        }
    ...
    list.asMap().forEach((i, dynamic e) {
      ...
      if (e is Map) {
        if (compact && _canFlattenMap(e)) {
          ...
          _printMap(e, initialTab: tabs + 1, isListItem: true, isLast: isLast);
        }
      } else {
